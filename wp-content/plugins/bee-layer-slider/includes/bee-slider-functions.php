...
function bee_layer_pro_notice() {
    ...
    <div class="notice  is-dismissible" >
       <a style="text-decoration:none;" href="http://beescripts.com/product/layer-slider" target="_blank"><img src="<?php echo plugin_dir_url( __FILE__ ).'../public/css/images/layer_pro.png'; ?>" alt="upgrade pro version" /></a>
    </div>
  ...
  background-size: cover;-webkit-transition: background-image 0.5s linear;">';
	...
	$bee_slide_txt = isset( $bee_slide_data['bee_text_layer'] ) ? $bee_slide_data['bee_text_layer'] : array();
		...
		$bee_img_step=0;
				...
				echo '<img class="bee-slide-img" 	src="'.$bee_slide_img.'"   data-position="'.$bee_img_pos_y.','.$bee_img_pos_x.'" data-in="'.$bee_img_anim.'" data-step="'.$bee_img_step.'" data-delay="'.$bee_img_delay.'" >';
$bee_img_step++;
//$bee_img_delay=$bee_img_delay+100;
		...
		$bee_txt_step= 1;
			...
			foreach ( $bee_slide_txt as $bee_txt_detail ) {
				...
				echo '<p 		class="bee-slider-text"  data-position="'.$bee_txt_pos_y.','.$bee_txt_pos_x.'" data-in="'.$bee_txt_anim.'" data-step="'.$bee_txt_step.'" data-delay="'.$bee_txt_delay.'" >'.$bee_txt_detail.'</p>';
...
function bee_options_page( $settings_pages )
{
	$settings_pages[] = array(
		...
		'menu_title'  => __( 'Bee Slider Options', 'textdomain' ),
